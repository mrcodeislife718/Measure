...
export default async function handler(req, res) {
  ...
  try {
    if (!await requireRateLimit(req, res, 'auth.login', { limit: 10, windowSeconds: 300 })) return;
    ...
    return res.status(200).json({ user: data.user ? { id: data.user.id, email: data.user.email } : null, authenticated: true, expiresIn: data.expires_in });
  } catch (error) {
