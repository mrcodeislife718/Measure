...
import { EvaluationEfficiencyLedger, failClosedQualification, rankExperimentsByInformationValue, stableEvidenceKey } from "../src/reliability-efficiency.js";
...
test("reuses evidence by stable digest", () => {
  ...
  assert.equal(ledger.reuseEvidence<{ passed: boolean }>(key)?.passed, true);
});
